...
**Роуты:**
```
/shop                    - каталог всех товаров
/shop/category/{slug}    - товары категории
/shop/my-purchases       - мои покупки (auth)
/shop/download/{id}      - скачивание файла (auth)
...
- Покупка за баланс
- Мои покупки с историей
- Прикреплённые файлы (доступны после покупки)

**Прикреплённые файлы:**
- Хранятся в `storage/app/shop/attachments/` (приватно)
- Поля в БД: `attachment` (путь), `attachment_name` (имя файла)
- Скачивание только для покупателей через `/shop/download/{id}`
- Макс. размер: 50 МБ
